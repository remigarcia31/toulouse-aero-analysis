    ...
    try:
        ...
        try:
            ...
            if data is None or 'states' not in data:
                 logging.warning("Réponse JSON vide ou sans champ 'states'. Aucune donnée à traiter.")
                 ...
                 if data['states'] is None or not data['states']:
                    logging.info("Champ 'states' est vide ou null. Aucun avion détecté dans la zone.")
                 else:
                    logging.info(f"{len(data['states'])} états d'avions reçus.")
                 raw_data = response.text # Utiliser .text pour garantir stockage du JSON brut
        ...
        gcs_file_path = f"{now.strftime('%Y/%m/%d/%H')}/states_{now.strftime('%Y%m%d_%H%M%S')}.json"
        ...
        logging.info(f"Données brutes OpenSky stockées dans : gs://{GCS_BUCKET_NAME}/{gcs_file_path}")
